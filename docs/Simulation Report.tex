...
Python libraries including PyTorch, OpenCV, and scikit-learn were used to implement model training, visual data processing, and computational components. The TraCI interface was employed to link SUMO with Python in real time.
...
Although functionally successful, this module was not integrated into the SUMO environment during the simulation phase. CameraVision integration with the live simulation environment is left as future work.
...
\begin{itemize}
    \item Queue lengths per lane: the number of vehicles waiting in each incoming lane.
    \item Number of halted vehicles: vehicles with a speed below 0.1 m/s, indicating congestion.
    \item Vehicle arrival rate: the number of vehicles entering each approach within a defined time window.
    \item Current signal phase: encoded as an integer representing the active traffic light configuration.
    \item Average approach speeds: the mean speed of vehicles across each lane group, used to estimate flow efficiency.
    \item Pressure metric: defined as $P = \sum(\text{incoming vehicles}) - \sum(\text{outgoing vehicles})$, representing the directional load at the intersection.
\end{itemize}
...
\begin{itemize}
    \item A mandatory yellow transition is enforced before any phase change.
    \item Each green phase must remain active for a minimum duration to prevent rapid flickering and ensure predictable driver behaviour.
    \item The agent is restricted from switching repeatedly between two phases within a short time interval.
\end{itemize}
...
\begin{itemize}
    \item A queue penalty: a negative reward proportional to total queue length across all lanes.
    \item A throughput reward: a positive signal for each vehicle that successfully exits the intersection.
    \item A halting penalty: additional penalties when large numbers of vehicles remain stationary.
    \item A phase-switching penalty: discourages unnecessary switching to promote smooth flow.
    \item A pressure reduction reward: encourages the agent to prioritise heavily congested approaches.
\end{itemize}

Taken together, these components push the agent toward reducing delays and keeping vehicles moving without switching signals so frequently that drivers cannot react predictably.
...
\begin{enumerate}
    \item The agent chooses a phase based on the current observation vector.
    \item TraCI applies the selected signal phase to the intersection.
    \item SUMO advances the simulation by a fixed number of timesteps to allow vehicles to react.
    \item Updated traffic data --- queues, speeds, counts --- are retrieved from SUMO.
    \item The environment computes the reward for the previous action.
    \item The next state and reward are returned to the agent for the next learning step.
\end{enumerate}

This cycle repeats for the duration of each episode. Over many episodes the agent accumulates enough experience to refine its policy from random phase selection toward one that consistently reduces queue lengths and waiting times.
