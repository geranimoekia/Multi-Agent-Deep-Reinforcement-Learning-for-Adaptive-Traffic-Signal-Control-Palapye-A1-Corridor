...
Figure~\ref{fig:methodology_overview} shows the high-level flow of the methodology from network construction through to policy evaluation.
...
\node[block, below=of train](eval)   {Policy Evaluation\\(Fixed-time vs PPO --- SUMO)};
...
\draw[arrow] (done.east) -- ++(1.6,0) node[above, font=\small]{No}
    |- ([yshift=0.3cm]obs.north east) -- (obs.north);

% Loop back: training not complete -> new episode
\draw[arrow] (conv.west) -- ++(-1.8,0) node[above, font=\small]{No}
    |- ([yshift=0.3cm]reset.north west) -- (reset.north);
