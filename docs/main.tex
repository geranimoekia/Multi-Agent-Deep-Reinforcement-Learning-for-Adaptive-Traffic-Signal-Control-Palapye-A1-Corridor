...
\begin{itemize}
    \item Design and implement a vehicle detection system using computer vision that accurately identifies vehicles, estimates queue lengths, and extracts real-time traffic metrics from camera footage, establishing the sensing foundation for data-driven traffic signal control.
...
\section{Project scope revision}

The original project scope, as proposed at the start of the final year, included a hardware phase alongside the software and simulation work. The plan was to prototype a physical traffic controller unit --- sensors, a local processor, and output relays --- to complement the simulation results.

During Semester 2, the project supervisor reviewed the direction of the work and approved a change of scope. The hardware implementation was set aside in favour of a more substantive algorithmic contribution: scaling the reinforcement learning system from a single isolated intersection to a coordinated three-intersection network using Multi-Agent Proximal Policy Optimisation (MAPPO). The rationale was that a well-designed multi-agent system addressing the full A1 corridor through Palapye, rather than a single junction, represents a stronger research contribution and is more directly applicable to the real network.

Accordingly, the four original objectives remain valid --- particularly the goals relating to intelligent optimisation algorithms and real-time adaptive control --- but the hardware sensing and output control objectives are reframed as future integration work. The Semester 2 deliverable is the MAPPO multi-intersection system, documented from Chapter~\ref{chap:mappo_system} onward.
...
Before reviewing control strategies, it is useful to establish the fundamental measures and relationships that define how traffic behaves at a signalised intersection. These metrics are what any control policy --- fixed-time or adaptive --- is ultimately trying to optimise.
...
\section{Traffic Simulation Tools}

Evaluating traffic signal controllers in a real network before deployment is impractical: live intersections cannot be arbitrarily reconfigured, and collecting sufficient training data from physical sensors is expensive and time-consuming. Traffic micro-simulators address this by providing synthetic, controllable environments in which vehicles are modelled individually, each obeying car-following and lane-change rules. The simulator generates the state information that the controller receives and carries out the control decisions the agent issues, closing the loop in software. The quality of the simulator --- its fidelity to real vehicle behaviour, its supported interaction interfaces, and its scalability to large networks --- therefore directly determines the quality of the learned policy.

\subsection{Overview of Traffic Simulators}

Several simulators are used in the research literature. VISSIM (PTV Group) is a commercial tool widely used for engineering consulting and detailed capacity analysis; it provides high visual fidelity and is calibrated against real-world data, but its closed-source nature and licensing costs limit its use in RL research. AIMSUN (TSS Transport Simulation Systems) is similarly commercial and widely used by road authorities for planning and evaluation. SUMO (Simulation of Urban Mobility) is an open-source, cross-platform micro-simulator maintained by the German Aerospace Center (DLR); it supports arbitrary road network topologies, multiple vehicle classes, and real traffic signal logic, and provides a Python API (TraCI) for real-time programmatic control during simulation --- a critical requirement for reinforcement learning integration. CityFlow is a more recent open-source simulator designed specifically for RL-based traffic signal control research; it is faster than SUMO for large grid networks but less feature-complete for non-standard network geometries \cite{futuretransp5040137}.

\subsection{SUMO as the Simulation Platform}

SUMO was selected for this project on the basis of three properties. First, its road network editor (NETEDIT) supports import from OpenStreetMap, which allowed the actual Palapye A1 corridor geometry to be used as the simulation base rather than an abstract grid. Second, TraCI exposes full programmatic control over traffic light phases, vehicle insertion, and sensor queries at every simulation step, enabling the RL environment to read intersection state and issue phase commands with sub-second temporal resolution. Third, SUMO is free, well-documented, and has an active community that maintains Python bindings compatible with major RL libraries.

SUMO models vehicle behaviour using the Krauss car-following model, which represents the longitudinal dynamics of each vehicle as a function of the gap to the vehicle ahead, the maximum deceleration, and a random perturbation term. Lane-change behaviour follows the SUMO lane-change model (LC2013), which decides when a vehicle changes lane based on strategic (reaching a destination lane), tactical (overtaking a slower vehicle), and safety constraints. Together these models produce realistic stop-and-go wave propagation and queue discharge behaviour that is qualitatively consistent with field observations at signalised intersections \cite{michailidis2025traffic}.

The TraCI interface allows a Python process to connect to a running SUMO instance, step the simulation forward one time step at a time, and exchange commands and state information at each step. For traffic signal control, the relevant TraCI calls are: \texttt{trafficlight.setRedYellowGreenState} to set phase, \texttt{lanearea.getLastStepHaltingNumber} to read queue length per lane, and \texttt{lanearea.getLastStepMeanSpeed} to read mean approach speed. These three calls are the primary data sources for the observation vector fed to the MAPPO agents in this project.
...
Adaptive control is the most capable category of traffic signal management. Unlike fixed-time and actuated methods, adaptive systems continuously monitor network-wide traffic density and apply algorithmic models to optimise signal timings across multiple intersections in real time \cite{mannion2016experimental}. The result is a controller that responds to conditions no preset schedule could anticipate --- sudden congestion, incidents, or atypical demand patterns that fall outside any programmed time-of-day profile.
...
MARL is formalised as a Decentralised Partially Observable Markov Decision Process (Dec-POMDP), in which each agent $i$ receives a local observation $o_i$ drawn from the global state $s$, chooses action $a_i$, and receives a team reward $r$. The joint policy $\boldsymbol{\pi} = \langle \pi_1, \ldots, \pi_n \rangle$ is optimised to maximise cumulative expected return for the group. Applications span robotics, game playing, autonomous vehicles, and networked infrastructure control --- anywhere that a set of interacting decision-makers must be coordinated without requiring a single central controller \cite{busoniu2008comprehensive}.
...
Three agents --- one per traffic light, labelled \texttt{TL\_A}, \texttt{TL\_B}, and \texttt{TL\_C} --- each have their own local observations. During training, a centralised critic receives the concatenated observations from all three agents and estimates the value of the joint state. This global view gives the critic enough information to assign credit correctly even when agents are making decisions simultaneously. At deployment, the critic is discarded and each agent runs only its own Actor network using local sensor data.
...
The Semester 1 PPO baseline (referred to as single-agent PPO or SA-PPO) treated all three traffic lights as one combined action space using Stable Baselines3. It had no shared reward and no cooperative objective. Table~\ref{tab:mappo_vs_ppo} summarises the qualitative differences in behaviour that emerge from the MAPPO architecture.
...
\toprule
\textbf{Property} & \textbf{Fixed-time} & \textbf{SA-PPO (Semester 1)} & \textbf{MAPPO (Semester 2)} \\
\midrule
...
Training time & N/A & $\sim$15 hours (5M steps) & $\sim$3--4 hours (1.5M steps) \\
\bottomrule
\end{tabular}
\end{table}
...
When two entry edges are blocked, the network experiences asymmetric demand with some approaches receiving no vehicles. Agents adapt by reducing green allocation to the empty approaches and consolidating time on the congested ones. This is a scenario where fixed-time control performs poorly --- it continues cycling through empty phases --- and where the adaptive advantage is most visible.
...
This project set out to address chronic congestion at Palapye's A1 corridor intersections by replacing static signal timing with an adaptive, learning-based controller. Work across two semesters produced two successive systems that demonstrate the potential of deep reinforcement learning for real-world traffic signal optimisation.

\paragraph{Semester 1 --- single-agent PPO}

A single Proximal Policy Optimisation agent was trained to control one isolated intersection in SUMO. Evaluated against a fixed-time baseline over 9,000 simulation timesteps, the PPO controller achieved a 48.5\% improvement in throughput ($p < 0.001$), a 27.8\% reduction in queue length ($p < 0.001$), and a 27.3\% reduction in traffic pressure ($p < 0.001$). Aggregate delay showed a marginal 10.1\% disadvantage relative to fixed-time control, but phase-specific analysis revealed delays 50--68\% lower during the heavy and peak demand periods that matter most in practice. These results confirmed that a reinforcement learning controller can learn demand-responsive phase allocation from scratch in simulation.

\paragraph{Semester 2 --- MAPPO multi-intersection system}

Following supervisor-approved scope revision, the system was extended to all three signalised intersections along the A1 Palapye corridor. Multi-Agent PPO with a Centralised Critic (MAPPO) was implemented under a Centralised Training, Decentralised Execution architecture. A shared Actor network, trained jointly on experience from all three agents, learns to coordinate phase timing across the network rather than optimising each junction in isolation. A cooperative reward signal, green wave pre-computation, and a three-stage curriculum over six demand scenarios were added to support learning in the more complex multi-intersection environment.

The MAPPO system produces behaviours not present in the single-agent baseline: cross-junction awareness, anticipatory discharge before downstream pressure builds, and coordination patterns resembling green wave progression during rush-hour conditions. These behaviours emerge from the reward structure and architecture rather than being explicitly programmed.

Together, the two systems validate the progression from a proof-of-concept single-intersection RL controller to a cooperative multi-agent system designed around the actual geometry and demand patterns of the Palapye road network.
...
The most immediate next step is integrating the YOLOv8-based vehicle detection module that was tested in Semester 1 as a standalone pipeline. At present, all training relies on synthetic traffic generated by SUMO. Feeding real queue-length estimates from a camera at one of the A1 junctions into the observation vector would allow the system to be evaluated on actual traffic conditions and would validate whether the learned policy transfers from simulation to reality.

The observation space can be enriched with turn-movement counts, which would allow the agent to distinguish between through and turning vehicles on each approach. The current 22-dimensional observation treats all lane vehicles uniformly; separating turn demands would give the agent more precise information for protected-turn phase decisions.

The reward function currently treats waiting time and queue length with fixed weighting. Incorporating vehicle emissions into the reward, so that long idling periods incur an additional penalty proportional to engine-on time, would make the optimisation objective closer to what matters for air quality. Palapye is a growing urban centre and fuel consumption at idling intersections is a tangible environmental cost.

Longer training with GPU acceleration would allow a higher total timestep budget and larger rollout batches. The current 1.5 million-step budget is practical for CPU training but is conservative. Training with 5--10 million steps would give the curriculum more time to expose the agents to rare edge cases like the incident scenario.

Finally, validating the system against real traffic count data from the Botswana Department of Roads, if such data can be obtained, would allow the simulation network to be calibrated to actual turning movements and flow rates, making the comparative evaluation against fixed-time control substantially more credible.
