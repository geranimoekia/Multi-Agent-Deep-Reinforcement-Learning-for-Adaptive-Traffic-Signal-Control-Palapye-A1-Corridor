...
# Every time the agent picks an action, we advance SUMO by 3 seconds.
# Shorter = more decisions per episode but slower training. 3s is a good balance.
DELTA_T       = 3

# YELLOW_DUR: how many RL steps the yellow light lasts before switching to the new green.
# In real traffic, yellow = warning light = ~3 seconds. With DELTA_T=3, 1 step ≈ 3 seconds.
...
# N_ACTIONS: each agent can choose one of 3 green phases at each step.
...
# As more episodes accumulate, harder traffic scenarios are introduced.
# This is called "curriculum learning" — start easy, add complexity once basics are learned.
#
#   Episode 0–29  : only "low" and "normal" traffic (gentle introduction)
#   Episode 30–79 : adds rush_hour_am, rush_hour_pm, holiday (moderate difficulty)
#   Episode 80+   : full random — any scenario including the hardest ones
#
# Each entry is (threshold, scenario_pool). The LAST threshold whose value ≤ episode_count wins.
CURRICULUM = [
    (0,  ["low", "normal"]),
