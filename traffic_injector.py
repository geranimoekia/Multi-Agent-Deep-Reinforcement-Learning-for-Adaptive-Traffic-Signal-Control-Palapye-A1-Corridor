...
def inject(step: int):
    """
    Attempt to inject one vehicle into the simulation at the current step.

    The scenario's demand_rate() returns a probability in (0, 1] for this step.
    We use it as a Bernoulli trial: if random() > rate, skip (no vehicle this step).
    When rate >= 1.0, always inject (used for very high-demand scenarios).

    CRITICAL BUG FIX (2025):
      _veh_count is incremented BEFORE the try block so every attempt uses a unique ID.
      Previous code only incremented on success, meaning a failed injection (e.g. SUMO
      rejects the vehicle for being too close to another) would retry with the same ID.
      After SUMO adds the route with route_id="route_veh_9", subsequent calls with the
      same route_id raise "route already exists" errors forever — an infinite retry loop.
      Always incrementing the counter guarantees globally unique IDs regardless of outcome.
    """
    ...
    # Get the injection probability for this scenario step, then advance the counter.
    # _scenario_step tracks how far into THIS scenario we are (resets on scenario change).
    rate = _scenario.demand_rate(_scenario_step)
    _scenario_step += 1

    # Stochastic injection: skip with probability (1 - rate)
    ...
    # This is the critical fix. Even if the injection fails, the next call uses a fresh ID.
    _veh_count += 1
